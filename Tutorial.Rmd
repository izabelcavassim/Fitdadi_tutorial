...
We start installing by dadi. If we look at the website for the dadi software [here](https://dadi.readthedocs.io/en/latest/user-guide/installation/), they recommend that we use the following:

```{bash, eval=F}
conda install -c conda-forge dadi
```

It requires installing multiple packages, you just need to type 'y' to proceed with the installation. 
To check whether dadi was sucessully installed we can just call the software on terminal:
...
You will see that we will get some errors because the Selection.py has its annotation in python2 and our environment is installed with python3. To fix this we will edit the Selection.py script manually, and for every 'print statement' we will include parenthesis as:
...
Ok, now that we have a function that exemplifies the specific demographic model we wanna test our data against we can now input the synonymous site frequency spectrum and use it to estimate our demographic parameters given the data.
